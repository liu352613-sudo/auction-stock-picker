...
| 模块 | 说明 |
| --- | --- |
| 精选选股 | 大盘温度仪表盘 + 指数 + 今日 Top3 + 全部候选；**实时刷新**指数/报价/热点板块与**盘中动态评分（▲/▼ delta）**，并展示数据日期与时效提示 |
| 股票详情 | 个股关键指标、**统一引擎 8 维度评分拆解**（量比/竞价金额/相对大盘/均线偏离/量能比/资金流/板块热度/趋势 + 风险调整）、Canvas K 线图（红涨绿跌）+ **实时报价/主力资金流/盘中实时重算评分** |
...
评分：统一引擎 `score_stock` 计算，含 8 个连续差异化子分项——量比(18)、竞价金额(12)、相对大盘(12)、均线偏离(15)、量能比(12)、资金流(12)、板块热度(10)、趋势(9)，权重合计 100，另含 ST/高波动/小市值/近涨停风险惩罚因子；缺失数据仅该项得 0，绝不出现固定 100 分。首页 / 历史 / 回测 / API 四端共用同一引擎与参数。详见 `docs/API.md` 第 2.1 节。
